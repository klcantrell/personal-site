...
export default function SplashView(rootEl, eventBus) {
  ...
  const splashSkills = rootEl.querySelector('.splash__skills');
  const heartPath = splashSkills.querySelector('#heart');
  ...
  function morphArrowOnScroll() {
    const morphArrowHandler = (() => {
      return k$throttle(k$whenTopTwoThirdsInView.bind(this, morphArrowToK, splashSkills), 800);
    })();
  ...
  function morphArrowToK() {
    if (!isArrowMorphed) {
      const morph = KUTE.to(arrowPath, {
        path: `M 152.77,315.36 C 229.84,278.14 252.00,249.33 252.00,229.39 252.00,216.53 243.14,209.00 228.97,209.00 205.48,209.00 171.38,227.17 134.61,264.39 134.61,264.39 107.58,404.00 107.58,404.00 107.58,404.00 81.89,404.00 81.89,404.00 81.89,404.00 141.69,95.22 141.69,95.22 141.69,95.22 168.27,93.00 168.27,93.00 168.27,93.00 139.48,240.02 139.48,240.02 175.36,203.48 212.58,185.00 240.92,185.00 265.95,185.00 280.00,199.03 280.00,219.64 280.00,248.44 253.08,281.69 185.11,315.36 185.11,315.36 269.50,401.78 269.50,401.78 269.50,401.78 239.44,406.00 239.44,406.00 239.44,406.00 152.77,315.36 152.77,315.36 Z`,
      }).start();
  ...
  const bigLetterMorphControl = (() => {
    const morph = KUTE.to(
      bigLetterK,
      {
        path: `M 266.00,210.86 C 266.00,210.86 217.00,211.58 217.00,211.58 217.72,177.94 214.09,166.00 197.41,166.00 164.77,166.00 143.00,217.38 143.00,258.61 143.00,305.63 172.03,319.02 245.67,295.86 245.67,295.86 240.23,341.47 240.23,341.47 142.80,373.69 91.00,341.83 91.00,266.20 91.00,189.48 143.86,120.00 208.30,120.00 252.94,120.00 268.89,149.33 266.00,210.86 Z`,
      },
  ...
  function resetBigLetterKPath() {
    bigLetterK.setAttribute(
      'd',
      `M 152.77,315.36 C 229.84,278.14 252.00,249.33 252.00,229.39 252.00,216.53 243.14,209.00 228.97,209.00 205.48,209.00 171.38,227.17 134.61,264.39 134.61,264.39 107.58,404.00 107.58,404.00 107.58,404.00 81.89,404.00 81.89,404.00 81.89,404.00 141.69,95.22 141.69,95.22 141.69,95.22 168.27,93.00 168.27,93.00 168.27,93.00 139.48,240.02 139.48,240.02 175.36,203.48 212.58,185.00 240.92,185.00 265.95,185.00 280.00,199.03 280.00,219.64 280.00,248.44 253.08,281.69 185.11,315.36 185.11,315.36 269.50,401.78 269.50,401.78 269.50,401.78 239.44,406.00 239.44,406.00 239.44,406.00 152.77,315.36 152.77,315.36 Z`,
    );
  }
  ...
  function loadFullProfilePic() {
    const rawResponsiveLoader = require('../../images/profile-pic-FIXED.jpg?sizes[]=800,sizes[]=1200');
